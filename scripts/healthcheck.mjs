...
export async function checkHealth({env=process.env}={}){
 ...
 const response=await fetch(`http://127.0.0.1:${port}/api/stats/public`,{
 ...
 if(!data||['completedJobs','verifiedFirms','ratingCount'].some(key=>!Number.isSafeInteger(data[key])||data[key]<0))throw Error('Invalid health response');
}
