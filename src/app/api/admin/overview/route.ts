...
export async function GET(_req: NextRequest) {
  ...
  return NextResponse.json({ jobs, firms, payments, bankPayouts, notifications:[...pushNotifications,...notifications].sort((a,b)=>String((b as Record<string,unknown>).created_at).localeCompare(String((a as Record<string,unknown>).created_at))).slice(0,100), proofs, reviews, stats });
}
