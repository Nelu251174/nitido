...
export async function GET(){if(!await isAdmin('manage'))return NextResponse.json({error:'Neautorizat'},{status:401});return NextResponse.json({drafts:catalogDrafts(db)});}
