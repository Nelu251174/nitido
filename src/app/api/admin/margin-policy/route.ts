...
export async function GET(){if(!await getAdminActorId())return response({error:'Neautorizat'},401);return response({policy:currentMarginPolicy(db),exceptions:marginExceptionReport(db),enabled:manualOffersEnabled()});}
