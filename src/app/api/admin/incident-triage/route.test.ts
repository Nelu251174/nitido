...
const state=vi.hoisted(()=>({db:null as Database.Database|null,actor:'verified-session' as string|null,origin:true,manage:true,audit:vi.fn()}));
vi.mock('@/lib/db',()=>({get db(){return state.db!;}}));
vi.mock('@/lib/adminAuth',()=>({getAdminActorId:async()=>state.actor,isAdmin:async()=>state.manage,auditAdminAction:state.audit}));
...
beforeEach(()=>{state.db=new Database(':memory:');state.db.exec('CREATE TABLE visit_cases(id TEXT PRIMARY KEY,updated_at TEXT);'+INCIDENT_TRIAGE_SCHEMA);state.actor='verified-session';state.origin=true;state.manage=true;state.audit.mockReset();});
...
it('keeps SLA policy publication restricted to management',async()=>{state.manage=false;expect((await POST(req(body()))).status).toBe(403);expect(state.db!.prepare('SELECT * FROM incident_sla_policy').all()).toEqual([]);});
