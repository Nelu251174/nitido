...
export async function GET(req:NextRequest){
 ...
 if(admin){if(!await getAdminActorId('operations'))return response({error:'Neautorizat'},401);}else{const user=await getCurrentUser(req);if(!user||user.role!=='client')return response({error:'Autentificare ca client necesară'},401);clientId=user.id;}
