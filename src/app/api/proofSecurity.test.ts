...
describe("proof upload and visibility security",()=>{
  ...
  it("derives proof ownership from authenticated allocated firm",()=>{expect(upload).toContain("executionAccess(db,user.id,jobId)");expect(upload).toContain("job.accepted_firm_id !== firm.id");expect(upload).toContain("uploaded_by_firm_id");expect(upload).not.toContain("formData.get(\"uploadedByFirmId\")");});
  it("requires allocated execution access for firm proof types",()=>{expect(upload).toContain("if (!access)");expect(upload).toContain("'CLIENT_CONTEXT'");});
