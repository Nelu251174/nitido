...
export async function GET(req:NextRequest){
 ...
 const executionRules=Object.fromEntries((db.prepare('SELECT r.job_id,r.items_json FROM job_execution_rules r JOIN jobs j ON j.id=r.job_id LEFT JOIN firms f ON f.id=j.accepted_firm_id WHERE j.client_id=? OR f.user_id=?').all(user.id,user.id) as {job_id:string;items_json:string}[]).map(r=>[r.job_id,JSON.parse(r.items_json)]));
