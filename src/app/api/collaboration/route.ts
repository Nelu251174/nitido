...
import { AccessError,acceptInvite,createInvite,revokeMember,resourceOwner,createApproval,decideApproval,executionAccess,submitExecutionReport,executionReportRecord } from "@/lib/collaborationAccess";
...
export async function GET(req:NextRequest){
 ...
 const execution=jobs.map(j=>({...j,checklist:db.prepare("SELECT item_key,done FROM workspace_checklist WHERE job_id=?").all(j.id),photos:db.prepare("SELECT id,proof_type FROM job_photos WHERE job_id=? AND status='VALID'").all(j.id),report:executionReportRecord(db,j.id)}));
