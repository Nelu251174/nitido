...
export async function GET(req:NextRequest){
 ...
 try{return response(hostPlan(db,user.id,requireText(req.nextUrl.searchParams.get('propertyId'),'Proprietate',100)));}catch(e){if(e instanceof WorkspaceError)return response({error:e.message},e.status);console.error('[host] read_failed');return response({error:'Calendarul nu poate fi încărcat.'},500);}
}
export async function POST(req:NextRequest){
 ...
 }catch(e){if(e instanceof WorkspaceError)return response({error:e.message},e.status);if(e instanceof SyntaxError)return response({error:'Cerere invalidă.'},400);console.error('[host] mutation_failed');return response({error:'Modificarea nu a putut fi salvată.'},500);}
}
