...
import {organizationSnapshot,saveOrganization,assignOrganizationProperty,inviteOrganization,revokeOrganizationAccess,OrganizationError} from '@/lib/organizations';
...
export async function GET(req:NextRequest){
 ...
 return response({organizations:organizationSnapshot(db,user.id),availableProperties:db.prepare(`SELECT p.id,p.name FROM workspace_properties p WHERE owner_id=? AND kind='business' AND archived=0 AND NOT EXISTS(SELECT 1 FROM workspace_organization_properties op WHERE op.property_id=p.id) ORDER BY p.name`).all(user.id)});
}
