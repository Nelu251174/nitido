...
import {FirmEarnings} from "@/components/FirmEarnings";
...
export default function FirmaPage() {
  ...
  const now = new Date();
  const confirmedJobs=historyJobs.filter(j=>j.financial?.paymentStatus==="captured"&&j.financial.refundStatus==="none"&&j.financial.disputeStatus==="none");
  const earningsTotal = confirmedJobs.reduce((sum,j)=>sum+(j.financial?.firmPayout??0),0);
  const earningsThisMonth = confirmedJobs
    .filter((j) => {
      return bucharestDay(j.completed_at??j.created_at).slice(0,7)===bucharestDay(now.toISOString()).slice(0,7);
    })
    .reduce((sum, j) => sum + (j.financial?.firmPayout??0), 0);
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <FirmSummary jobs={myJobs}/></div><section id="lucrari-active">
          ...
          <div className="space-y-3">
            {activeJobs.map((job) => (
              <Card key={job.id}>
                {(() => { const arrivalCount=job.proofs?.filter(p=>p.type==="ARRIVAL").length??0,completionCount=job.proofs?.filter(p=>p.type==="COMPLETION").length??0; const photoRules=job.photoRules??{arrivalMin:1,completionMin:1}; const hasArrival=arrivalCount>=photoRules.arrivalMin,hasCompletion=completionCount>=photoRules.completionMin; return <>
                ...
                {job.status === "accepted" && (
                  <div className="space-y-3"><div className="rounded-xl border border-line bg-mist p-4"><div className="flex justify-between gap-3 text-sm font-bold"><span>Fotografii la sosire · minimum {photoRules.arrivalMin}</span><span className={hasArrival?"text-aqua-deep":"text-coral"}>{arrivalCount} / {photoRules.arrivalMin}</span></div><label className="mt-3 block cursor-pointer rounded-lg border border-line bg-white px-4 py-2 text-center text-sm font-bold">{uploadingProof===`${job.id}:ARRIVAL`?"Se încarcă…":"Încarcă fotografie la sosire"}<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={Boolean(uploadingProof)} onChange={e=>void uploadProof(job.id,"ARRIVAL",e.target.files?.[0])}/></label></div><Button disabled={!hasArrival} className="w-full disabled:cursor-not-allowed disabled:opacity-50" onClick={() => markArrived(job.id)}>Am ajuns / Începe lucrarea</Button><button onClick={() => cancelJob(job.id)} className="w-full text-xs font-display font-bold text-coral py-1">Renunță la lucrare (o repunem pentru altă firmă)</button></div>
                )}
                {job.status === "arrived" && (
                  <div className="space-y-3"><div className="rounded-xl border border-line bg-mist p-4"><div className="flex justify-between gap-3 text-sm font-bold"><span>Fotografii la finalizare · minimum {photoRules.completionMin}</span><span className={hasCompletion?"text-aqua-deep":"text-coral"}>{completionCount} / {photoRules.completionMin}</span></div><label className="mt-3 block cursor-pointer rounded-lg border border-line bg-white px-4 py-2 text-center text-sm font-bold">{uploadingProof===`${job.id}:COMPLETION`?"Se încarcă…":"Încarcă fotografia finală"}<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={Boolean(uploadingProof)} onChange={e=>void uploadProof(job.id,"COMPLETION",e.target.files?.[0])}/></label></div><p className="text-xs leading-5 text-muted">Plata este blocată până la finalizarea corectă a lucrării. Pentru eliberarea plății sunt obligatorii fotografiile de finalizare prevăzute pentru această lucrare.</p><Button disabled={!hasCompletion || Boolean(settlingJob)} className="w-full disabled:cursor-not-allowed disabled:opacity-50" onClick={() => markComplete(job.id)}>Finalizează lucrarea și solicită încasarea</Button></div>
                )}
                </>})()}
              </Card>
            ))}
        ...
        <FirmEarnings revision={JSON.stringify(historyJobs.map(j=>[j.id,j.financial]))}/>
        <AppRatingCard />
