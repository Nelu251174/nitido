...
describe("Stripe split architecture",()=>{
  ...
  it("never accepts price, payout, or destination from API request bodies",()=>{const jobRoute=fs.readFileSync("src/app/api/jobs/route.ts","utf8");const paymentCode=fs.readFileSync("src/lib/payments.ts","utf8");expect(jobRoute).toContain("calcServicePrice(spaceType, sqm,windowsSqm)");expect(jobRoute).not.toMatch(/const priceGross\s*=\s*body\./);expect(paymentCode).not.toMatch(/body\.(amount|payout|destination)|application_fee_amount|transfer_data/);expect(paymentCode).toContain("destination:String(row.stripe_account_id)");});
  it("shows client total and firm server-derived payout without exposing secrets",()=>{const client=fs.readFileSync("src/app/client/page.tsx","utf8");const firm=fs.readFileSync("src/app/firma/page.tsx","utf8");expect(client).toContain("job.price_gross");expect(firm).not.toContain("job.price_gross");expect(firm).toContain("job.firm_payout");expect(client+firm).not.toMatch(/STRIPE_SECRET_KEY|client_secret/);});
});
