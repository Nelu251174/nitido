...
export type PublicManualOffer={id:string;assessmentId:string;revision:number;status:string;expiresAt:string;createdAt:string;terms:{evidence?:{reviewId:number;photoIds:string[]};currency:'RON';scope:string;context:{category:string;city:string;sqm:number};lines:{label:string;amountBani:number}[];grossBani:number;discountBani:number;totalBani:number}};
