...
export type EarningRow={id:string;clientName:string;street:string;city:string;completedAt:string;arrivedAt:string|null;estimatedMinutes:number;gross:number|null;commission:number|null;net:number|null;paymentStatus:string|null;refundStatus:string|null;disputeStatus:string|null;transferStatus:string|null;payoutStatus:string|null;day:string;recordedMinutes:number|null;confirmedNet:number};
export function earningsForMonth(db:Database,userId:string,month:string):EarningRow[]{
 ...
 p.amount_gross AS gross,p.commission_amount AS commission,p.amount_net AS net,p.status AS paymentStatus,p.refund_status AS refundStatus,p.dispute_status AS disputeStatus,p.transfer_status AS transferStatus,p.payout_status AS payoutStatus
