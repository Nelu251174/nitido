...
const method = (extra = {}) => ({ id: "pm_new", type: "card", customer: "cus_owner", card: { last4: "3155" }, ...extra });
...
describe("saved card replacement", () => {
  it("opens setup for the existing customer, preserving the old card until completion", async () => {
    ...
    expect(mocks.createSession).toHaveBeenCalledWith(expect.objectContaining({ mode: "setup", customer: "cus_owner", client_reference_id: "owner", metadata: { previousPaymentMethodId: "pm_old" }, success_url: "https://sandbox.nitido.ro/client?card=added&session_id={CHECKOUT_SESSION_ID}" }));
  ...
  it("supports the first card without a previous default", async () => {
    db.exec("UPDATE users SET stripe_payment_method_id=NULL");
  ...
  it("does not overwrite a newer selection when an older tab returns", async () => {
    ...
    await expect(confirm()).rejects.toThrow("între timp");
