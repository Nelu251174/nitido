...
export function HostTurnover({properties,events,checks,jobs,links,selected,onSelect,onCheck,busy}:{properties:Property[];events:Stay[];checks:HostCheck[];jobs:JobRow[];links:{property_id:string;job_id:string}[];selected:string;onSelect:(id:string)=>void;onCheck:(payload:Record<string,unknown>,success:string)=>Promise<boolean>;busy:boolean}){
 ...
 return <section className="design-panel host-turnover" id="pregatire"><div className="board-card-heading"><h2><DesignIcon name="calendar"/>Verificarea pregătirii proprietății</h2><select aria-label="Proprietate pentru pregătirea sejurului" value={property.id} onChange={e=>{onSelect(e.target.value);setEventId('')}}>{properties.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select></div>
  {current&&<label className="host-stay-picker">Pregătire după plecarea oaspeților<select value={current.id} onChange={e=>setEventId(e.target.value)}>{stays.map(s=><option value={s.id} key={s.id}>{date(s.starts_at)} – {date(s.ends_at)}</option>)}</select></label>}
  ...
  <div className="host-preparation-layout"><div><h3>{property.name}</h3><div className="host-turnover-grid"><article><DesignIcon name="user"/><small>Plecare oaspeți</small><h3>{current?date(current.ends_at):'Adaugă o perioadă'}</h3><p>Eliberare din calendar</p></article><article><DesignIcon name="broom"/><small>Curățenie</small><h3>{cleaning?.scheduled_at?date(cleaning.scheduled_at):'Neprogramată'}</h3><p>{end?`Până la ${date(end)}`:'Alege un interval din propunerile de mai sus.'}</p><Link className="board-soft-button" href={cleaning?'/client#sec-lucrari':'#plan-curatenie'}>{cleaning?'Vezi rezervările':'Vezi propunerile'} <DesignIcon name="arrow" size={18}/></Link></article><article><DesignIcon name="home"/><small>Sosire oaspeți</small><h3>{next?date(next.starts_at):'Neconfirmată'}</h3><p>Conform calendarului importat</p></article></div>{current&&next&&<p className="host-window">Fereastră între sejururi: {Math.round((Date.parse(next.starts_at)-Date.parse(current.ends_at))/60000)} minute{cleaning?` · Curățenie estimată: ${cleaning.duration_minutes} minute`:''}</p>}</div>
