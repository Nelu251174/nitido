...
export function BookingEstimator({options}:{options:EstimatorOption[]}){
 ...
 return <form className="design-search" aria-label="Configurare rapidă" onSubmit={e=>{e.preventDefault();router.push(`/rezervare?${new URLSearchParams({city,sqm,date,spaceType:type})}`)}}>
  <label className="design-search-field"><DesignIcon name="pin" variant="booking"/><span><span className="design-field-label">Localitate</span><input aria-label="Localitate" list="nitido-cities" value={city} onChange={e=>setCity(e.target.value)} required maxLength={100}/><datalist id="nitido-cities">{CITIES.map(c=><option key={c.slug} value={c.name}/>)}</datalist></span></label>
  ...
  <label className="design-search-field"><DesignIcon name="calendar" variant="booking"/><span><span className="design-field-label">Alege data</span><input aria-label="Alege data" type="date" value={date} onChange={e=>setDate(e.target.value)}/></span></label>
