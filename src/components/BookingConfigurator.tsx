...
import {WindowsExtra} from './WindowsExtra';
import {useRef,useState} from 'react';
import Link from 'next/link';
import {useRouter} from 'next/navigation';
import {DesignIcon} from './DesignIcon';
import {calcServicePrice,calcServiceDuration,calcWindowsPrice,validWindowsSqm,priceExplanation,SLOT_HOURS,type SpaceType} from '@/lib/pricing';
...
export function BookingConfigurator({initial,options}:{initial:Record<string,string>;options:EstimatorOption[]}){
 ...
 const [city,setCity]=useState((initial.city??'').slice(0,100));
 const [windowsSqm,setWindowsSqm]=useState(validWindowsSqm(Number(initial.windowsSqm))?Number(initial.windowsSqm):0);
 ...
 const valid=validWindowsSqm(windowsSqm)&&Number.isInteger(Number(area))&&Number(area)>=10&&Number(area)<=1000;
 const price=valid?calcServicePrice(type,Number(area),windowsSqm):null,duration=valid?calcServiceDuration(Number(area),windowsSqm):null;
 ...
 function validate(all=false){
  ...
  if(!validWindowsSqm(windowsSqm))return 'Introdu suprafața geamurilor între 0 și 200 m², fără zecimale.';
 ...
 function submit(){
  ...
  const params=new URLSearchParams({spaceType:type,sqm:area,city:city.trim(),mode,windowsSqm:String(windowsSqm)});
 ...
 return <div className="booking-wizard">
  ...
  <form className="booking-grid" noValidate onSubmit={e=>{e.preventDefault();submit()}}>
   ...
   <aside className="booking-summary design-panel"><p className="booking-eyebrow">ALEGERILE TALE</p><h2>Rezumatul curățeniei</h2><div className="booking-summary-line"><DesignIcon name="home"/><div><b>{label}</b><p>{area||'—'} m² · {city.trim()||'Alege localitatea'}</p></div></div><div className="booking-summary-line"><DesignIcon name="calendar"/><div><b>{scheduled?dateLabel:'Cât mai curând'}</b><p>{scheduled?(hour?`${hour.padStart(2,'0')}:00 · ora României`:'Alege ora'):'În funcție de disponibilitate'}</p></div></div><div className="booking-summary-line"><DesignIcon name={mode==='express'?'bolt':'check'}/><div><b>{mode==='express'?'Express':'Standard'}</b><p>{mode==='express'?'Prima firmă eligibilă care acceptă':'Compari și alegi firma'}</p></div></div><div className="booking-total"><div><b>Total estimat</b><p>Curățenie + extraopțiuni selectate</p></div><output aria-live="polite">{price??'—'} lei</output></div><p className="booking-muted">{priceExplanation(type,Number(area))}. Numărul camerelor nu se taxează separat.</p>{validWindowsSqm(windowsSqm)&&<p className="booking-muted">Spălare geamuri: {windowsSqm} m² · {calcWindowsPrice(windowsSqm)} lei, separat de curățenie.</p>}{duration&&<p className="booking-duration"><DesignIcon name="clock" size={18}/>Durată estimată: {Math.floor(duration/60)} h{duration%60?` ${duration%60} min`:''}</p>}<p className="booking-muted">Estimarea folosește tariful curent. Prețul final și creditul eligibil se verifică în cont înainte de publicare.</p><div className="booking-summary-assurance"><DesignIcon name="shield"/><span>Nu se încasează bani în configurator.</span></div></aside>
  </form>
 </div>;
}
