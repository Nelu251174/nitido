...
// Tipurile de spațiu disponibile în estimatorul live (client poate alege).
const ESTIMATOR_TYPES:SpaceType[]=["apartament","casa","birou","altul"];
...
export function LivePriceEstimator(){
  ...
  return <div className="absolute bottom-[-20px] right-[-18px] w-[300px] rounded-2xl bg-[#101711] p-5 text-white shadow-2xl max-sm:right-3 max-sm:w-[calc(100%-24px)]">
    ...
    <div className="mt-2 flex flex-wrap gap-1.5" role="group" aria-label="Tip spațiu">
      {ESTIMATOR_TYPES.map(type=><button key={type} type="button" onClick={()=>setSpaceType(type)} aria-pressed={spaceType===type} className={`rounded-full px-2.5 py-1 text-[11px] font-bold transition ${spaceType===type?"bg-[#39c97c] text-[#101711]":"bg-[#2a332c] text-[#a8b2ac] hover:text-white"}`}>{PROPERTY_TYPE_LABELS[type]}</button>)}
    </div>
    <div className="mt-3 flex items-end justify-between gap-4"><span className="text-sm text-[#a8b2ac]">{PROPERTY_TYPE_LABELS[spaceType]} · <output htmlFor="hero-sqm" className="font-semibold text-white">{sqm} m²</output></span><b className="shrink-0 text-3xl tabular-nums" aria-live="polite">{price} lei</b></div>
