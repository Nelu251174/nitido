...
import {activeEstimatorOptions} from "@/lib/estimatorConfig";
...
// Tipurile afișate în estimator vin din configul unic src/lib/estimatorConfig.ts
// (ușor de modificat: adaugi/ascunzi/redenumești acolo, într-un singur loc).
const ESTIMATOR_OPTIONS=activeEstimatorOptions();

export function LivePriceEstimator(){
  const [spaceType,setSpaceType]=useState<SpaceType>(ESTIMATOR_OPTIONS[0]?.key??"apartament");
  const [sqm,setSqm]=useState(ESTIMATOR_DEFAULT_SQM);
  const price=calcGrossPrice(spaceType,sqm);
  const activeLabel=ESTIMATOR_OPTIONS.find(o=>o.key===spaceType)?.label??"Apartament";
  ...
  return <div className="absolute bottom-[-20px] right-[-18px] w-[300px] rounded-2xl bg-[#101711] p-5 text-white shadow-2xl max-sm:right-3 max-sm:w-[calc(100%-24px)]">
    ...
    <div className="mt-2 flex flex-wrap gap-1.5" role="group" aria-label="Tip spațiu">
      {ESTIMATOR_OPTIONS.map(option=><button key={option.key} type="button" onClick={()=>setSpaceType(option.key)} aria-pressed={spaceType===option.key} className={`rounded-full px-2.5 py-1 text-[11px] font-bold transition ${spaceType===option.key?"bg-[#39c97c] text-[#101711]":"bg-[#2a332c] text-[#a8b2ac] hover:text-white"}`}>{option.label}</button>)}
    </div>
