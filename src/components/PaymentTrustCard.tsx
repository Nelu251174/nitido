...
export function PaymentTrustCard() {
  return <article className="v2-card h-full p-5 lg:p-6"><div className="v2-eyebrow">PLATĂ PROTEJATĂ</div><h3 className="mt-3 text-2xl font-bold">Plata rămâne controlată până la final.</h3><p className="mt-4 text-sm leading-6 text-[#64748b]">Fluxul este configurat pentru autorizare cu capturare manuală. Suma poate rămâne rezervată pe card, sub formă de hold / preautorizare, numai după confirmarea procesatorului.</p>
    ...
    <div className="mt-5 rounded-2xl bg-[#111827] p-5 text-white"><div className="text-xs font-bold text-[var(--nitido-brand-on-dark)]">EXEMPLU DE AUTORIZARE</div><div className="mt-2 text-3xl font-bold">500 lei</div><p className="mt-2 text-sm leading-6 text-[#c2cbc5]">Suma rămâne rezervată pe cardul clientului pe durata lucrării după confirmarea autorizării de către procesator.</p><p className="mt-3 text-sm leading-6 text-white">După apăsarea butonului „Finalizează lucrarea”, platforma inițiază capturarea. Plata este marcată capturată numai după răspunsul reușit al procesatorului.</p><p className="mt-3 text-xs leading-5 text-[#9ca8a0]">Timpul efectiv până când fondurile apar în contul bancar al firmei poate depinde de procesatorul de plăți și de bancă.</p></div>
    <ol className="mt-4 grid gap-2 text-xs sm:grid-cols-3">{steps.map(([number,title,copy])=><li key={title} className="rounded-xl border border-[#d8d7d0] bg-[#faf9f5] p-3"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--nitido-brand-soft)] font-bold text-[var(--nitido-brand-dark)]">{number}</span><b className="mt-2 block text-[10px] leading-4 text-[var(--nitido-brand-dark)]">{title}</b><p className="mt-1.5 leading-4 text-[#64748b]">{copy}</p></li>)}</ol>
  </article>;
}
