...
export function SupportCenter() {
  ...
  return <>
    ...
    <section className="bg-white"><div className="v2-container py-20"><div className="grid grid-cols-2 gap-5 max-md:grid-cols-1"><SupportCard icon="phone" title="Preferi să vorbești cu noi?" value="0341.402.403" text="Echipa NITIDO te poate ajuta în situațiile care necesită suport uman." href="tel:0341402403" action="Sună acum"/><SupportCard icon="mail" title="Scrie-ne" value="contact@nitido.ro" text="Pentru întrebări generale, documente sau situații care necesită analiză detaliată." href="mailto:contact@nitido.ro" action="Trimite email"/></div>{status?.authenticated && status.supportHref && <div className="mt-5 flex items-center justify-between gap-6 rounded-[20px] bg-[#111827] p-7 text-white max-sm:flex-col max-sm:items-start"><div><h3 className="text-xl font-bold">Suport pentru o lucrare existentă</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-[#b8c1bb]">Dacă problema este legată de o lucrare activă, folosește conversația și mesajele asociate lucrării pentru ca echipa și participanții autorizați să aibă contextul corect.</p></div><Link href={status.supportHref} className="v2-btn shrink-0 bg-[#39c97c] text-[#111827]">Deschide panoul {status.role === "firma" ? "firmei" : "client"}</Link></div>}</div></section>
    <PrivacyNotice/><Faq/>
  </>;
}

function SupportCard({icon,title,value,text,href,action}:{icon:"phone"|"mail";title:string;value:string;text:string;href:string;action:string}){return <article className="rounded-[22px] border border-[#e2e8f0] bg-[#f7f9fc] p-8"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#e4f0e8] text-[#115e59]">{icon === "phone" ? <Phone/> : <Mail/>}</span><h3 className="mt-8 text-2xl font-bold">{title}</h3><a href={href} className="mt-3 block text-xl font-bold text-[#115e59]">{value}</a><p className="mt-4 max-w-lg text-sm leading-6 text-[#64748b]">{text}</p><a href={href} className="v2-btn v2-btn-primary mt-7">{action}</a></article>}
function PrivacyNotice(){return <section className="v2-container py-16"><div className="grid grid-cols-[auto_1fr] gap-5 rounded-[20px] border border-[#c9dfd1] bg-[#e8f5f2] p-7 max-sm:grid-cols-1"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white text-[#115e59]"><Shield/></span><div><h2 className="text-xl font-bold">Confidențialitate în conversație</h2><p className="mt-3 leading-7 text-[#3e4842]">Asistentul AI NITIDO nu îți va cere parola, codurile de autentificare sau datele complete ale cardului.</p><p className="mt-2 leading-7 text-[#3e4842]">Datele personale sunt folosite numai în limitele necesare pentru suport și conform permisiunilor contului autentificat.</p></div></div></section>}
function Faq(){const items=[["Cum postez o lucrare?","Autentifică-te ca client, deschide panoul client și selectează „Postează o lucrare”. Completează spațiul, locația, programarea și fotografiile opționale."],["Cum este aleasă firma?","La Standard, clientul alege dintre firmele candidate. La Express, prima firmă eligibilă care acceptă prin server primește lucrarea."],["Când vede firma adresa exactă?","Numai firma căreia i-a fost alocată lucrarea vede detaliile exacte necesare execuției. Înainte de acceptare, adresa rămâne protejată."],["Cum funcționează plata?","Plata este urmărită prin stări distincte. O autorizare eșuată nu alocă fals lucrarea, iar acțiunile de plată rămân în fluxurile securizate."],["Ce se întâmplă dacă firma nu se prezintă?","Clientul poate raporta no-show pentru propria lucrare, conform stării acesteia. Incidentul este înregistrat și plata autorizată este anulată prin fluxul securizat."],["Cum contactez suportul?","Sună la 0341.402.403 sau scrie la contact@nitido.ro. Pentru o lucrare activă, folosește și mesajele asociate din cont."],["Cum îmi șterg sau modific contul?","Poți modifica datele profilului din cont, prin Editează profilul. Pentru ștergerea contului, contactează suportul la 0341.402.403 sau contact@nitido.ro pentru verificare și ajutor."]];return <section className="bg-white"><div className="v2-container py-20"><div className="v2-eyebrow">SUPORT RAPID</div><h2 className="v2-h2 mt-4">Răspunsuri la întrebările esențiale</h2><div className="mt-10 grid grid-cols-2 gap-4 max-md:grid-cols-1">{items.map(([q,a])=><details key={q} className="group rounded-2xl border border-[#e2e8f0] bg-[#f7f9fc] p-6"><summary className="cursor-pointer list-none pr-8 font-bold marker:hidden">{q}<span className="float-right text-[#0f766e] group-open:rotate-45">+</span></summary><p className="mt-4 text-sm leading-6 text-[#64748b]">{a}</p></details>)}</div></div></section>}
