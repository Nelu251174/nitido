    ...
    <section id="asistent-ai" className="v2-container pt-0 pb-0">
      <div className="grid min-h-fit grid-cols-[.72fr_1.28fr] items-start gap-10 max-[1000px]:grid-cols-1">
        ...
        <div className="overflow-hidden rounded-[24px] border border-[#d8d7d0] bg-white shadow-[0_28px_80px_rgba(16,23,17,.12)]">
          ...
          <div ref={chatMessagesRef} aria-live="polite" className="h-[430px] space-y-4 overflow-y-auto bg-[#faf9f5] p-6 max-sm:h-[390px] max-sm:p-4">{messages.map((message,index)=><div key={`${message.role}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "rounded-br-md bg-[var(--nitido-brand)] text-white" : "rounded-bl-md border border-[#e2e8f0] bg-white text-[#29322d]"}`}>{message.content}</div></div>)}{loading&&<div className="flex justify-start"><div className="rounded-2xl rounded-bl-md border border-[#e2e8f0] bg-white px-4 py-3 text-sm text-[#64748b]"><span className="inline-flex gap-1"><i className="h-2 w-2 animate-pulse rounded-full bg-[var(--nitido-brand)]"/><i className="h-2 w-2 animate-pulse rounded-full bg-[var(--nitido-brand)] [animation-delay:150ms]"/><i className="h-2 w-2 animate-pulse rounded-full bg-[var(--nitido-brand)] [animation-delay:300ms]"/></span></div></div>}</div>
