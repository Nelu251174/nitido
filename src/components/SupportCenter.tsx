...
const items = ids.map(id => SUPPORT_TOPICS.find(topic => topic.id === id)!).map(topic => [topic.title, topic.answer]);return <section className="bg-white"><div className="v2-container py-20"><div className="v2-eyebrow">SUPORT RAPID</div><h2 className="v2-h2 mt-4">Răspunsuri la întrebările esențiale</h2><div className="mt-10 grid grid-cols-2 gap-4 max-md:grid-cols-1">{items.map(([q,a])=><details key={q} className="group rounded-2xl border border-[#e2e8f0] bg-[#f7f9fc] p-6"><summary className="cursor-pointer list-none pr-8 font-bold marker:hidden">{q}<span className="float-right text-[#0f766e] group-open:rotate-45">+</span></summary><p className="mt-4 text-sm leading-6 text-[#64748b]">{a}</p></details>)}</div></div></section>}
