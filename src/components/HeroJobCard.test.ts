...
describe("dynamic hero job card",()=>{
  it("derives title from centralized service mapping",()=>{expect(card).toContain("jobTypeLabel(job.serviceType)");expect(card).not.toContain(">Curățenie apartament<");expect(homepage).toContain("<BookingEstimator options={getActiveEstimatorOptions(db)}/>");expect(homepage).not.toContain("estimatedPrice:");});
