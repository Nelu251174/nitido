...
export function AdminExecutionTemplates(){
 const [photoRules,setPhotoRules]=useState({arrivalMin:1,completionMin:1});
 ...
 function choose(t:ExecutionTemplate){setSelected(t);setPhotoRules({...t.photoRules});setItems(t.items.map(i=>({...i})));setReason('');setMessage('');}
 ...
 return <section className="design-panel my-5 space-y-4" style={{background:'#f7f3ec',minWidth:0}}>
  ...
  {selected&&<form className="space-y-4" onSubmit={e=>{e.preventDefault();void save();}}>
   ...
   <fieldset className="grid gap-3 sm:grid-cols-2"><legend className="font-bold">Fotografii obligatorii pentru serviciul ales</legend><label>Minimum la sosire<input className={inputClass} type="number" min={1} max={20} required disabled={busy} value={photoRules.arrivalMin} onChange={e=>setPhotoRules({...photoRules,arrivalMin:Number(e.target.value)})}/></label><label>Minimum la finalizare<input className={inputClass} type="number" min={1} max={20} required disabled={busy} value={photoRules.completionMin} onChange={e=>setPhotoRules({...photoRules,completionMin:Number(e.target.value)})}/></label></fieldset>
   <p className="text-sm">Se numără doar fotografiile validate ale echipei alocate. Cerințele se păstrează în lucrare și la remediere. Fotografii private: evită fețe, documente, coduri de acces și alte date personale.</p>
