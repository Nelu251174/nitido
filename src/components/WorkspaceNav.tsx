...
export function WorkspaceNav({role='client',onNavigate}:{role?:'client'|'firma'|'admin';onNavigate?:(href:string)=>void}){
 ...
 ]:[['Operațiuni','/admin','home'],['Firme','/admin#firme','building'],['Lucrări','/admin#lucrari','briefcase'],['Calitate','/admin#calitate','shield'],['Plăți','/admin#plati','card'],['Catalog','/admin#catalog','settings'],['Evaluări','/admin#evaluari','briefcase'],['Website public ↗','/?spatiu=admin','home']];
 ...
 return <nav className="workspace-nav" aria-label={`Navigare ${role}`}>{primary.map(([label,href,icon])=>{const [targetPath,targetHash='']=href.split('#');const selected=path===targetPath&&hash===(targetHash?'#'+targetHash:'');const opensWebsite=role==='admin'&&href==='/?spatiu=admin';const NavLink=opensWebsite||targetHash||targetPath===path?'a':Link;return <NavLink key={href} href={href} target={opensWebsite?'_blank':undefined} rel={opensWebsite?'noopener noreferrer':undefined} aria-label={opensWebsite?'Website public — se deschide într-o filă nouă':undefined} onClick={()=>onNavigate?.(href)} aria-current={selected?'page':undefined}><DesignIcon name={icon} size={22}/><span>{label}</span></NavLink>})}{role==='client'&&<details className="professional-nav" open={professional.some(([,href])=>path===href)}><summary>Opțiuni profesionale</summary><p>Ai o firmă, închiriezi proprietăți sau ai primit o invitație? Alege spațiul potrivit.</p>{professional.map(([label,href,description])=><Link key={href} href={href} onClick={()=>onNavigate?.(href)} aria-current={path===href?"page":undefined}><span><b>{label}</b><small>{description}</small></span></Link>)}</details>}</nav>;
}
