...
function WebAlertsSession(){
 ...
 function chime(){const ctx=sound.current;if(!ctx||ctx.state!=='running'||Date.now()-lastSound.current<3000)return;lastSound.current=Date.now();const oscillator=ctx.createOscillator(),gain=ctx.createGain();oscillator.connect(gain);gain.connect(ctx.destination);oscillator.frequency.setValueAtTime(740,ctx.currentTime);oscillator.frequency.setValueAtTime(980,ctx.currentTime+.12);gain.gain.setValueAtTime(.08,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+.35);oscillator.start();oscillator.stop(ctx.currentTime+.36);oscillator.onended=()=>{oscillator.disconnect();gain.disconnect();};}
 useEffect(()=>{
  ...
  const refresh=async()=>{if(busy)return;busy=true;try{
   ...
   const claim=()=>{if(!alive)return;let seen=memory.get(key)??null;try{const stored=localStorage.getItem(key);if(stored){const parsed=JSON.parse(stored);if(Array.isArray(parsed)&&parsed.every(x=>typeof x==='string'))seen=parsed;}}catch{}
    ...
    setToast({title,href,count:result.fresh.length});if(on)chime();
 ...
 return <aside className="web-alerts" aria-label="Alerte NITIDO"><button className="web-alert-toggle" onClick={()=>void enable()}>{enabled?'Oprește sunetul și alertele desktop':'Activează sunetul și alertele desktop'}</button>{hint&&<p className="web-alert-hint" role="status">{hint}<button aria-label="Închide explicația" onClick={()=>setHint('')}>×</button></p>}{toast&&<div className="web-alert-toast" role="status"><button aria-label="Închide alerta" onClick={()=>setToast(null)}>×</button><strong>{toast.title}</strong>{toast.count>1&&<p>{toast.count} noutăți în cont.</p>}<a href={toast.href}>Vezi detaliile</a></div>}</aside>;
}
