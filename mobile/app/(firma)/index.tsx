...
export default function FirmHome(){
  ...
  <SectionTitle title="Organizare"/><PrimaryButton secondary title="Calendar și echipe" onPress={()=>router.push("/(firma)/teams")}/><PrimaryButton secondary title="Verificări lucrări" onPress={()=>router.push("/(firma)/execution")}/><PrimaryButton secondary title="Mesaje" onPress={()=>router.push("/(firma)/messages")}/></AppScreen>;
}
