...
import { canCompleteJob, canStartJob, completeFirmJob, firmOperationError, hasProof, markFirmArrived, navigationUrl, paymentState } from "@/firmOperations";
...
import { canShareTracking, stopTracking, updateTracking } from "@/tracking";

export default function Active(){
  const {jobs,loading,error:jobsError,reload}=useJobs();
  const [busy,setBusy]=useState<string|null>(null);
  const [error,setError]=useState<string|null>(null);
  const [failed,setFailed]=useState<{asset:PhotoAsset;type:"ARRIVAL"|"COMPLETION"}|null>(null);
  ...
  async function chooseProof(type:"ARRIVAL"|"COMPLETION",camera:boolean){if(!job||busy)return;setBusy(`proof-${type}`);setError(null);let asset:PhotoAsset|null=null;try{const picker=await import("expo-image-picker");const permission=camera?await picker.requestCameraPermissionsAsync():await picker.requestMediaLibraryPermissionsAsync();if(!permission.granted)throw new Error(camera?"Permite accesul la cameră pentru dovada foto.":"Permite accesul la galerie pentru dovada foto.");const result=camera?await picker.launchCameraAsync({mediaTypes:["images"],quality:.8}):await picker.launchImageLibraryAsync({mediaTypes:["images"],quality:.8});if(result.canceled)return;asset=result.assets[0];await uploadFirmProof(asset,job.id,type);setFailed(null);await reload()}catch(cause){if(asset)setFailed({asset,type});setError(firmOperationError(cause))}finally{setBusy(null)}}
  async function retryProof(){if(!job||!failed||busy)return;setBusy(`proof-${failed.type}`);setError(null);try{await uploadFirmProof(failed.asset,job.id,failed.type);setFailed(null);await reload()}catch(cause){setError(firmOperationError(cause))}finally{setBusy(null)}}
  ...
  async function complete(){if(!job||busy||!canCompleteJob(job))return;setBusy("complete");setError(null);try{await completeFirmJob(job.id,api);const detail=await api<{job:Job}>(`/api/jobs/${job.id}`);setFinished(detail.job);setConfirming(false);await reload()}catch(cause){setError(firmOperationError(cause));setConfirming(false);await reload()}finally{setBusy(null)}}
  async function toggleTracking(){if(!job||job.status!=="arrived")return;if(sharing){locationSubscription.current?.remove();locationSubscription.current=null;await stopTracking(job.id,api);setSharing(false);return}setError(null);try{const Location=await import("expo-location");const permission=await Location.requestForegroundPermissionsAsync();if(permission.status!=="granted")throw new Error("Permite locația numai în timpul lucrării pentru a activa urmărirea.");locationSubscription.current=await Location.watchPositionAsync({accuracy:Location.Accuracy.Balanced,timeInterval:15_000,distanceInterval:25},position=>{void updateTracking(job.id,{latitude:position.coords.latitude,longitude:position.coords.longitude,accuracy:position.coords.accuracy},api).catch(()=>setError("Locația nu a putut fi actualizată. Verifică internetul."))});setSharing(true)}catch(cause){setError(firmOperationError(cause))}}
  ...
  if(finished)return <AppScreen eyebrow="FINALIZARE CONFIRMATĂ" title="Lucrarea a fost finalizată"><PremiumCard style={s.success}><Ionicons name="checkmark-circle" size={48} color={colors.green}/><Text style={s.cardTitle}>{jobTypeLabel(finished.space_type)}</Text><Text style={s.detailText}>{paymentState(finished)}</Text><Text style={s.payment}>Confirmarea din platformă nu reprezintă sosirea instantanee a fondurilor în contul bancar.</Text></PremiumCard><PrimaryButton secondary title="Vezi încasările" icon="wallet-outline" onPress={()=>{setFinished(null)}}/></AppScreen>;
  ...
  return <AppScreen eyebrow="EXECUȚIE ÎN SIGURANȚĂ" title={jobTypeLabel(job.space_type)} subtitle={`${job.city} · ${job.sqm} m²`}>
    ...
    <Proof title="Fotografie la sosire" loaded={arrival}/><View style={s.buttonGroup}><PrimaryButton secondary disabled={arrival||busy!==null} loading={busy==="proof-ARRIVAL"} icon="camera-outline" title={arrival?"Fotografie confirmată":"Cameră · sosire"} onPress={()=>void chooseProof("ARRIVAL",true)}/>{!arrival?<PrimaryButton secondary disabled={busy!==null} icon="images-outline" title="Galerie · sosire" onPress={()=>void chooseProof("ARRIVAL",false)}/>:null}</View>
    {failed?.type==="ARRIVAL"?<PrimaryButton secondary loading={busy==="proof-ARRIVAL"} icon="refresh" title="Reîncearcă dovada de sosire" onPress={()=>void retryProof()}/>:null}
    <PrimaryButton disabled={!canStartJob(job)||busy!==null} loading={busy==="arrived"} title="Am ajuns / Începe lucrarea" onPress={()=>void start()}/>
    <Proof title="Fotografie la finalizare" loaded={completion}/><View style={s.buttonGroup}><PrimaryButton secondary disabled={completion||job.status!=="arrived"||busy!==null} loading={busy==="proof-COMPLETION"} icon="camera-outline" title={completion?"Fotografie confirmată":"Cameră · finalizare"} onPress={()=>void chooseProof("COMPLETION",true)}/>{!completion?<PrimaryButton secondary disabled={job.status!=="arrived"||busy!==null} icon="images-outline" title="Galerie · finalizare" onPress={()=>void chooseProof("COMPLETION",false)}/>:null}</View>
    {failed?.type==="COMPLETION"?<PrimaryButton secondary loading={busy==="proof-COMPLETION"} icon="refresh" title="Reîncearcă dovada finală" onPress={()=>void retryProof()}/>:null}
...
function Proof({title,loaded}:{title:string;loaded:boolean}){return <PremiumCard style={s.proof}><View style={[s.proofIcon,loaded&&{backgroundColor:colors.green}]}><Ionicons name={loaded?"checkmark":"camera-outline"} size={21} color={loaded?colors.white:colors.green}/></View><View style={{flex:1}}><Text style={s.proofTitle}>{title} · OBLIGATORIU</Text><Text style={s.proofState}>{loaded?"Încărcată și validată de server":"Lipsă"}</Text></View><Pill label={loaded?"Încărcată":"Lipsă"} tone={loaded?"green":"red"}/></PremiumCard>}
