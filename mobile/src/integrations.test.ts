...
describe("final mobile integrations",()=>{
  ...
  it("keeps SMS backend-only",()=>{expect(source("app/notifications.tsx")).toContain("SMS-ul de rezervă este gestionat exclusiv de backend");expect(source("src/pushClient.ts")).not.toMatch(/sendSms|twilio|SMS_API_KEY/)});
